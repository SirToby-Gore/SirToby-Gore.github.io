...
export class App {
    ...
    addLooseItem(name, amount, unit = 'each', category = 'general') {
        if (!name || amount <= 0)
            return;
    ...
    `;
        ...
        const addBtn = formRow.querySelector('#add-loose-2');
        const nameInput = formRow.querySelector('#loose-name-2');
        const amountInput = formRow.querySelector('#loose-amount-2');
        const unitSelect = formRow.querySelector('#loose-unit-2');
        const categorySelect = formRow.querySelector('#loose-category-2');
        addBtn.onclick = () => {
            ...
            if (existing && !this.isUnitCompatible(existing.unit, unit)) {
                window.alert(`Unit "${unit}" is not compatible with existing unit "${existing.unit}" for "${name}".`);
                return;
            }
    ...
    attachCollapseHandlers(container) {
        const toggles = Array.from(container.querySelectorAll('.collapse-toggle'));
        toggles.forEach((btn) => {
    ...
    isUnitCompatible(existing, incoming) {
        if (existing === incoming)
            return true;
        const mass = ['g', 'kg'];
        const volume = ['ml', 'l'];
        const each = ['each', 'pc'];
        if (mass.includes(existing) && mass.includes(incoming))
            return true;
        if (volume.includes(existing) && volume.includes(incoming))
            return true;
        if (each.includes(existing) && each.includes(incoming))
            return true;
        return false;
    }
    removeAggregatedItem(name) {
        ...
        for (const idStr of recipeIds) {
            ...
            if (!recipe)
                continue;
            const uses = recipe.ingredients.some((ing) => ing.ingredient.name === name);
            if (uses) {
