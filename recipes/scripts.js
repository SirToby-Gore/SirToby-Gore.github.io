...
class App {
    recipes = [
        {
            ...
            description: 'Classic British sausages in Yorkshire pudding batter.',
            timeMinutes: 40,
            servings: 4,
            ingredients: [
        ...
        },
        {
            ...
            description: 'A sweet and savoury dish featuring tender turkey and juicy peaches.',
            timeMinutes: 30,
            servings: 3,
            ingredients: [
    ...
    bindSearch() {
        ...
        if (!input)
            return;
        input.oninput = () => this.viewHome(document.getElementById('app'), input.value);
    }
    addRecipe(id) {
    ...
    viewHome(root, query = '') {
        const recipes = this.recipes.filter((r) => r.name.toLowerCase().includes(query.toLowerCase()));
