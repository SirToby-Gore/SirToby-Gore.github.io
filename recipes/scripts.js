...
class Ingredient {
    ...
    allergens;
    amount;
    constructor(name, unit = 'each', category = 'general', allergens = [], amount = 0) {
        this.name = name;
        this.unit = unit;
    ...
    getAmount(inputAmount, inputUnit) {
        ...
        if (!sourceConversion || !targetConversion) {
            throw new Error(`Unknown unit conversion for ${sourceUnit} or ${targetUnit}`);
        }
        if (sourceConversion.group !== targetConversion.group) {
            throw new Error(`Incompatible units for "${this.name}": Cannot convert ${sourceUnit} (${sourceConversion.group}) to ${targetUnit} (${targetConversion.group})`);
        }
    ...
    static asparagusSpears = new Ingredient('Asparagus Spears', 'each', 'produce');
    ...
    static dicedGreenBellPepper = new Ingredient('Diced Green Bell Pepper', 'each', 'produce');
    static icebergLettuce = new Ingredient('Iceberg Lettuce', 'each', 'produce');
    ...
    static choppedTomatoes = new Ingredient('Chopped Tomatoes', 'each', 'pantry');
    static redKidneyBeans = new Ingredient('Red Kidney Beans', 'each', 'pantry');
    ...
    static groundBlackPepper = new Ingredient('Ground Black Pepper', 'tsp', 'spices');
    ...
    static groundCoriander = new Ingredient('Ground Coriander', 'tsp', 'spices');
    ...
    static mildCurryPowder = new Ingredient('Mild Curry Powder', 'tsp', 'spices');
    ...
    static worcestershireSauce = new Ingredient('Worcestershire Sauce', 'ml', 'pantry', ['fish']);
    static dijonMustard = new Ingredient('Dijon Mustard', 'tsp', 'pantry');
    static whiteWineVinegar = new Ingredient('White Wine Vinegar', 'ml', 'pantry');
    ...
    static strongBreadFlour = new Ingredient('Strong Bread Flour', 'g', 'pantry', ['gluten']);
    ...
    static fingerBiscuits = new Ingredient('Savoiardi Biscuits', 'g', 'pantry', ['gluten', 'eggs']);
    ...
    static springRollWrappers = new Ingredient('Spring Roll Wrappers', 'each', 'pantry', ['gluten']);
...
class App {
    ...
    sortEntries(entries) {
        ...
        if (this.sortMode === 'category')
            return entries.sort((a, b) => a[1].category.localeCompare(b[1].category));
        return entries;
    }
    updateBasketCount() {
    ...
    viewBasket(root) {
        ...
        const recipeListHtml = Object.entries(this.basket).length === 0
            ? '<p class="dim-text">No recipes added.</p>'
            : Object.entries(this.basket)
                .map(([idStr, count]) => {
                const r = this.recipes.find((rec) => rec.id === parseInt(idStr));
                return r
                    ? `
					<div class="list">
